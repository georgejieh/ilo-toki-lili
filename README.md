...
Phase 0 is in progress. The repo structure, locked Python baseline, local hooks, CI, module hygiene, Linku source policy, frozen `data/vocab.json`, closed-vocabulary tokenizer, pydantic data schemas, and contamination checks are in place. Next up: config hashing and the remaining reproducibility guardrails.
